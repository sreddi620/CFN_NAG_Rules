...
class SecurityGroupIngresswithOutPort443Rule < BaseRule
  def rule_text
  ...
  def audit_impl(cfn_model)
    violating_security_groups = cfn_model.security_groups.select do |security_group|
      violating_egresses = security_group.egresses.select do |egress|
        egress.fromPort != egress.toPort && egress.toPort.to_s != 443
      end
    ...
    violating_egresses = cfn_model.standalone_egress.select do |standalone_egress|
      standalone_egress.fromPort != standalone_egress.toPort && standalone_egress.toPort.to_s != 443
    end
